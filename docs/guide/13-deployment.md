...
> **확인:** 빌드 산출물에 remoteEntry.js 가 생성됐는지 반드시 확인하세요. 이 파일이 없으면 호스트가 리모트를 로드할 수 없습니다. 누락 시 [14-troubleshooting](./14-troubleshooting.md) 의 "remoteEntry.js 생성 안 됨" 항목을 참고하세요.
...
> 📍 **사내 레지스트리 등록 후 배포는 ONM에서 수행하세요.**

위 2단계(사전 조건 + 레지스트리 IP 설정)로 사내 레지스트리 등록(푸시)까지 끝났다면 실제 배포는 **ONM** 에서 진행합니다.
...
| 해시 붙은 번들 (`/ext/assets/*-[hash].js`, `.css`) | `immutable, 1y` | 해시 파일명이라 영구 캐시 안전 |
