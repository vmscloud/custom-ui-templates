...
1. **신규 분석/조회 화면**: Trino/Iceberg 에 쌓인 대용량 시계열(`rpt_`_, `odv\_`_ 등)을 피벗·필터·그리드 형태로 보여주는 페이지.
...
이 저장소의 코드가 실행되는 환경은 2가지 입니다.


| 환경                            | 언제              | 프론트 URL                            | Host 주입                    | 세션                  | 비고                                                   |
| ----------------------------- | --------------- | ---------------------------------- | -------------------------- | ------------------- | ---------------------------------------------------- |
| **Host 통합 (운영서버 혹은 개발서버 배포)** | APS에 실제 얹혀 있을 때 | `https://...aps/ext/...` 등 APS 라우팅 | `provide('hostData', ...)` | 있음 (fusionauth.sid) | 실제 서비스 동작. `/api/aps/...` 포함 전부 200                  |
